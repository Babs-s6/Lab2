document.addEventListener('DOMContentLoaded', function () {
  $(function () {
    ...
    /* L.map inserts a map with a setView (choosing the coordiantes)  */
    ...
    /*L.tileLayer helps to load and display tile layers on the map. */

    let markers = L.markerClusterGroup({
        removeOutsideVisibleBounds: false
    });
    mymap.addLayer(markers);
    /*L.marker used to display clickable icons on the map. */
    ...
    document.querySelector('button').onclick = function () {
        ...
        markers.clearLayers(); /*Removes all the layers from the group, except From date to To date */
        ...
        request.onload = function () {
            ...
            console.log(data);
            /*What information we want to show in the popup*/
            for (let permit of data.features) {
                ...
                const popupinfo = `Issue date: ${issuedate}<br>Work Class Group: ${p.workclassgroup}<br>Contractor Name: ${p.contractorname}<br>Community Name: ${p.communityname}<br>Original Address: ${p.originaladdress}<br>Status: ${p.statuscurrent}<br>Estimate Project Cost in $: ${p.estprojectcost}`;
                if (!permit.geometry){
                    continue;
                }
